"""Natural linguistic feature probe for the quartic detection story.

This C10 probe extends the external-label C9 experiment beyond surface lexical
events. Labels are rule-based linguistic/semantic token events (closed-class
words, pronouns, modals, negation, suffix-based nouns/verbs/adjectives, months,
and title words). A direction is learned from train labels and evaluated on a
held-out split with the same label/full-covariance/off-diagonal detector-class
audit used in C9.
"""
...
def clean_token(token: str) -> str:
    ...
    stripped = stripped.replace("Ġ", "").replace("▁", "")
...
def analyze(args: argparse.Namespace, x: np.ndarray, events: dict[str, np.ndarray], meta: dict[str, object]) -> dict[str, object]:
    ...
    write_csv(outdir / "natural_linguistic_feature_probe.csv", rows)
    if not rows:
        raise RuntimeError("No linguistic events passed the filters.")
    ...
    summary = {
        "mode": "natural_linguistic_feature_probe",
        **meta,
        ...
        "rows": rows,
        "output_csv": str(outdir / "natural_linguistic_feature_probe.csv"),
        "output_figure": str(outdir / "natural_linguistic_feature_probe.pdf"),
    }
    with (outdir / "natural_linguistic_feature_probe_summary.json").open("w", encoding="utf-8") as f:
    ...
    try:
        ...
        fig.tight_layout()
        fig.savefig(outdir / "natural_linguistic_feature_probe.png", dpi=220)
        fig.savefig(outdir / "natural_linguistic_feature_probe.pdf")
        plt.close(fig)
...
def main() -> None:
    ...
    parser.add_argument("--outdir", default="results/real_activation/c10_natural_linguistic_feature_probe")
